...
function measureCell(cell, rowHeight) {
  ...
  if (cell.image) {
    const image = this.document.openImage(cell.image);
    let { width, height } = image;

    // Ignore orientation based on document options or image options
    const imageOptions = cell.imageOptions || {};
    const ignoreOrientation =
      imageOptions.ignoreOrientation ||
      (imageOptions.ignoreOrientation !== false &&
        this.document.options.ignoreOrientation);
    ...
    let w = imageOptions.width || width;
    let h = imageOptions.height || height;

    if (imageOptions.width && !imageOptions.height) {
      const wp = w / width;
      w = width * wp;
      h = height * wp;
    } else if (imageOptions.height && !imageOptions.width) {
      const hp = h / height;
      w = width * hp;
      h = height * hp;
    } else if (imageOptions.scale) {
      w = width * imageOptions.scale;
      h = height * imageOptions.scale;
    } else if (imageOptions.fit || cell.fit) {
      const [bw, bh] = imageOptions.fit || cell.fit;
      const bp = bw / bh;
      ...
      // If no explicit size, scale down to fit column width if necessary
      if (w > textAllocatedWidth) {
        const wp = textAllocatedWidth / width;
        w = width * wp;
        h = height * wp;
      }
    }
    textBounds = { x: 0, y: 0, width: w, height: h };
  } else if (cell.text) {
