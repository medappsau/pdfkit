...
function renderCell(cell, rowStruct) {
  const cellRenderer = () => {
    ...
    if (cell.image) {
      ...
      // Auto fit to cell
      if (!opts.fit && !opts.width && !opts.height && !opts.scale && !opts.cover) {
        opts.fit = [cell.width - left - right, cell.height - top - bottom];
      }

      // Align
      if (!opts.align && align && align.x && align.x !== 'left') {
        opts.align = align.x;
      }
      if (!opts.valign && align && align.y && align.y !== 'top') {
        opts.valign = align.y;
      }

      this.document.image(cell.image, cell.x + left, cell.y + top, opts);
    } else if (cell.text) renderCellText.call(this, cell);
  };
